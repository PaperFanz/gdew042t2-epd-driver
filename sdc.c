...
// **********SDCard Pins*******************
// 1  Unconnected
// 2  PC4 SDC_CS, active low to enable SDC
// 3  PA5 SDC MOSI
// 4  Vdd +3.3V
// 5  PA2 Sclk
// 6  Vss Gnd
...
void SSI0_Init(uint32_t CPSDVSR){
  ...
  GPIO_PORTA_AMSEL_R &= ~0x34;          // disable analog functionality on PA2,4,5
	TFT_CS = TFT_CS_HIGH;									// REMOVE THIS AND ADD EPD DEFINES (They both are mapped to same location rn though)
  //EPD_CS = EPD_CS_HIGH;                 // disable LCD
  // initialize SSI0
...
static int select(void){
	//EPD_CS = EPD_CS_HIGH; // TRY TO SWITCH TO THIS (TFT_CS line does the same thing but name isn't right)
													// add EPD defines into gdew042t2.h?
